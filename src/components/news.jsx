    ...
    const fetchNews = async () => {
      ...
      if (!error && data) {
        const formattedNews = data.map((item, index) => {
          const colors = ["bg-red-500", "bg-blue-500", "bg-orange-500"];
          return {
            id: item.id,
            title: item.title,
      ...
      <section className="relative z-10 mx-auto  rounded-br-4xl rounded-bl-4xl ">
        <div className="text-center mb-8 md:mb-12 mx-auto px-4 sm:px-6 md:px-8 lg:px-16">
          <h2 className="text-3xl md:text-4xl lg:text-5xl font-bold tracking-wide pt-6 md:pt-9 flex flex-col sm:flex-row justify-center items-center text-yellow-500 font-merri">
            ...
            <BellRing
              stroke="#EAB308"
              className="w-6 h-6 sm:w-6 sm:h-6 md:w-8 md:h-8 lg:w-9 lg:h-9 mt-2 sm:mt-0 sm:ml-3 lg:opacity-100 "
            />
          </h2>
        ...
        {loading ? (
          ...
          <div className="text-center py-20 text-gray-500 font-nuno text-lg">
        ...
        <div className="text-center pb-10 md:pb-18">
          <button
            onClick={() => handleNews()}
            className="bg-gradient-to-l from-[#f97316] via-[#e11d48] to-[#ef4444] text-white px-6 md:px-8 py-2 md:py-3 rounded-full font-medium transition flex items-center gap-2 mx-auto cursor-pointer text-sm md:text-base hover:scale-105"
          >
